...
int main3()
{
    ...
    ads_vectorAppend(los,"Sourav Das",const char*);
    ads_vectorAppend(los,"GOURAV DAS",const char*);
    ads_vectorAppend(los,"RITA DAS",const char*);
    char * cs;
    ads_vectorGetItem(los,0, char*,cs);
...
int main2()
{
    ...
    ads_avlInsert(tos,6,int,"SOURAV DAS",const char*);
    ads_avlInsert(tos,2,int,"SOURAV DAS",const char*);
    ads_avlInsert(tos,9,int,"SOURAV DAS",const char*);
...
int main5()
{
    ...
    ads_dictSetItem(d2,"Sourav Das",const char*,1,int);
    ads_dictSetItem(d2,"Gourav Das",const char*,2,int);
    ads_dictSetItem(d2,"Rita Das",const char*,3,int);
    ads_dictSetItem(d2,"Subhas Das",const char*,4,int);
    ads_dictSetItem(d2,"Arati Das",const char*,5,int);
...
int main6() //6
{
    ...
    ads_heapBuilt(t,const char*,"Apple", "Ball", "Cat","Dog", "Egg", "Fish","Aeroplane" );  /*insert some elements*/
...
int main8()//8
{
    ...
    ads_stackBuilt(s,const char*,"apple","mapple");
...
#include "..\Source\Ads\str.h"
#include "..\Source\Ads\utility\timeit.h"


int main10()//10
{
    ads_str s = ads_strFrmDouble(10.20,10,3);
    ads_str s1 = ads_new(ads_str,"Sourav Das %d %s",10,"");
    ads_str s5 = ads_new(ads_str,"Go Baby %s","");
...
int main14()//14
{
    ...
    ads_tupleAppendTD(t,ads_cstrTD);
    ads_tupleAppend(t,"Sourav Das",const char*);
    //ads_tupleSetItem(t,0,32,int);
    //ads_tupleSetItem(t,1,32,int);

    char* k;
    ads_tupleGetItem(t,2,char*,k);
...
int extractBit(float f,int bit)
{
	int32_t t ;
	memcpy(&t,&f,sizeof(f));
	return (int)(t>>bit)&0x1;
}
...
int main()
{
	...
	ads_str exp = ads_strNew("%s"," ");
	ads_str mantissa = ads_strNew("%s" ," ");

	ads_str s = ads_strFrmDouble(f,2,75);

	ads_printf("in binary %str \nsign = %d  ",s,extractBit(f,31));

	for(i=30;i>22;i--)
	{
		ads_strCat(exp,"%d",extractBit(f,i));
	}
	i = ads_strToInt64(exp,2);
	ads_printf("\nexp = {str} biased = %d , actual = %d",exp,i,i-127);

	for(i=22;i>=0;i--)
	{
		ads_strCat(mantissa,"%d",extractBit(f,i));
	}
	ads_printf("\nmantissa = {str} %f",mantissa,f);

	printf("\n f= %f,%g",f,f);

	ads_delete(exp,mantissa,s);
}
